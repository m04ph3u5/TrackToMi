...
public class AppServiceImpl implements AppService{
	...
	private void saveGeofence(DetectedPosition last, DetectedPosition p, String userId) {
		//se la distanza tra il punto di fine di un viaggio e quello di inizio del nuovo viaggio è maggiore di 3000 m allora li considero entrambi geofence
		//in caso contrario salvo come geofence soltanto il punto di fine del viaggio che sto per salvare
		
		if(p.getPosition().getLat()!=NOPOSITION && distFrom(last.getPosition().getLat(), last.getPosition().getLng(), p.getPosition().getLat(), p.getPosition().getLng())>MAX_GEOFENCE_DISTANCE){
